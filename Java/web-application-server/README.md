...
### Servlet
- init()
  - 호출되는 시점 : 컨테이너가 서블릿 파일을 찾아서 시작할 때 서블릿 인스턴스를 생성한 후 init() 을 호출.
  - 목적 : 클라이언트의 요청을 처리하기 전에 서블릿을 초기화
    - 초기화 할 코드가 있다면 재정의를 할 수 있다. (데이터베이스에 대한 접속, 다른 객체에 서블릿을 등록 등)
  - ServletConfig 객체와 ServletContext 객체에 접근할 수 있다. 이 두 객체를 통해 서블릿 및 웹 애플리케이션 설정 정보를 파악할 수 있음
- service()
  - 호출되는 시점 : 최초 클라이언트의 요청을 받았을 때 컨테이너는 새로운 스레드를 생성하거나 아니면 스레드 풀로부터 하나를 가지고 와서 서블릿의 service() 메소드를 호출.(init()이후 호출)
  - 목적 : 클라이언트의 HTTP 메소드 (GET, POST 등)을 참조하여 doGET(), doPOST()를 호출할지 판단

- DD(Deployment Descriptor) 배포 서술자 
  - 웹 컨테이너에게 사용자가 지금 접근한 URL 주소가 서블릿 요청임을 인식하고 그 서블릿 클래스의 위치는 어디에 있는지 알려주기 위해 필요한 정보들이 적혀있는 파일.  
  - 주로 `web.xml`에 작성된다.
