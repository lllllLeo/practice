...
int er;
er = ((Integer)session.getAttribute("suc")).intValue();
if(er==0){
 		/*
...
<div class="img_ratio_58223_">
	...
	try {
		...
		<center>
		<table>
		<% for(i=0; i<adlist.size(); i++){  %> <!--데이터가 있으면 true 없으면 false...없을때까지 while문이 돈다  -->
		<%if(adlist.get(i).getStore_name().equals("전국점포")){ %>
			
		<% }else{ %>
		<tr>
		<td align = center ><font size="5px"><b><% out.println(j+1); %>.</b></font></td>
		<td align = center><font size="5px"><b><%= adlist.get(i).getStore_name() %>&nbsp</b></font></td>
		
		<td align="center"><input type="button" class="button1" onClick="location.href=
		'admindelete.ad?store_name=<%= adlist.get(i).getStore_name()%>'"  value="삭제"></td>		
		</tr><!-- 각각의 데이터에 값을 넣어준다.  -->
		<tr>
			<td colspan="3"><hr></td>
			</tr>
	    ...
	    <tr>
	    <td colspan="3" align="center"><input type="button" class="button" onClick="location.href='admin.ad'"  value="이전"></td>
	    </tr>
		</table>
		<%
