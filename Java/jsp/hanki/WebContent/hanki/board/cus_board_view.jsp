...
<%
QnaBean qna = (QnaBean)request.getAttribute("qnabean");
%>
<!DOCTYPE HTML>
<html lang="ko">
<head>
...
<div id="farmBoxLnb" class="lnb_183742_">
   ...
   <ul class="dep1">
         ...
         <a href="/bbs/g1.yu" target="_self">Free Board</a>
         <div>
            <!-- 2 Depth -->
            <ul class="dep2">
                                             <li class=" on">
...
<!--       </div>
   ...
   <div class="det">
      ...
      <div class="hgroup">
         <div class="title">
            <strong><%=qna.getQ_title()%></strong>
         </div>
            <dl>
               <dt>
                  <span><%=qna.getQ_name() %></span><span><%=qna.getQ_location()%></span>
                  <span><%=qna.getQ_email1()%><%=qna.getQ_email2()%><%=qna.getQ_email3()%></span>
                  <span><%=qna.getQ_phone1()%>-<%=qna.getQ_phone2()%>-<%=qna.getQ_phone3()%></span>
               </dt>
            <%-- <dd><span><%=qna. %></span></dd> --%>
            </dl>
         </div>
...
function editArticle(){
                  var edit_win = window.open('../bbs_shop/popup/pwd_chk_form.htm?pwd_mode=board_edit&me_popup=&auto_frame=&cate_sub_idx=0&list_mode=board&board_code=g1&keyfield=&key=&page=0&idx=278081','edit_win','top=150,left=300,width=330,height=450,scrollbars=no');
         edit_win.focus();
...
function editComment(bbs_read_idx,idx){
      ...
      var window_left = (screen.width-w)/2;
      var window_top = (screen.height-h)/2;
      var editComment_win = window.open("../bbs_shop/popup/comment_edit_form.htm?board_code=g1&board_idx="+bbs_read_idx+"&idx="+idx,"editComment_win","top="+window_top+",left="+window_left+",width="+w+",height="+h+",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
      editComment_win.focus();
   }

function replyComment(bbs_read_idx,idx){
         var w = 400;
      var h = 380;
      var window_left = (screen.width-w)/2;
      var window_top = (screen.height-h)/2;
      var replyComment_win = window.open("../bbs_shop/popup/comment_reply_form.htm?board_code=g1&board_idx="+bbs_read_idx+"&idx="+idx,"replyComment_win","top="+window_top+",left="+window_left+",width="+w+",height="+h+",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
      replyComment_win.focus();
   }


function guest_del(bbs_read_idx,idx){
         var w = 330;
      var h = 200;
      var window_left = (screen.width-w)/2;
      var window_top = (screen.height-h)/2;
      var del_win = window.open("../bbs_shop/popup/pwd_chk_form.htm?pwd_mode=comment_del&board_code=g1&board_idx="+bbs_read_idx+"&idx="+idx,"del_win","top="+window_top+",left="+window_left+",width="+w+",height="+h+",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
      del_win.focus();
   }

function guest_edit(bbs_read_idx,idx){
         var w = 330;
      var h = 200;
      var window_left = (screen.width-w)/2;
      var window_top = (screen.height-h)/2;
      var edit_win = window.open("../bbs_shop/popup/pwd_chk_form.htm?pwd_mode=comment_edit&board_code=g1&board_idx="+bbs_read_idx+"&idx="+idx,"edit_win","top="+window_top+",left="+window_left+",width="+w+",height="+h+",toolbar=no,scrollbars=yes,resizable=yes,status=yes,menubar=no,location=no");
      edit_win.focus();
   }

function comment_submitForm(bbs_read_idx){

   var frm = document.getElementsByName('frmComment_'+bbs_read_idx)[0];

         frm.target=create_iframe();
      frm.method="post";
      frm.action="../bbs_shop/comment_reg_exec.php";

                  if (frm.write_name.value.split(" ").join("") == ""){
               alert("작성자 이름을 입력해주세요");
               frm.write_name.focus();
               return;
            }
            if (frm.passwd.value.split(" ").join("") == ""){
               alert("암호를 입력해주세요");
               frm.passwd.focus();
               return;
            }

            if (frm.content.value.split(" ").join("") == ""){
               alert("내용을 입력해주세요");
               frm.content.focus();
               return;
            }
            if (frm.content.value.split(" ").join("").length > 1000){
               alert("1000자 이하만 댓글 가능합니다");
               frm.content.focus();
               return;
            }
            frm.submit();
...
function comment_default_str_chk(obj){
   if(!obj) return;
   if(obj.value == ""){
      obj.value = "";
   }
