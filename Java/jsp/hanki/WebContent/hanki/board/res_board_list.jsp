...
<%
	List ResBoardlist=(List)request.getAttribute("ResBoardlist");
	int listcount=((Integer)request.getAttribute("listcount")).intValue();
	int nowpage=((Integer)request.getAttribute("page")).intValue();
	int maxpage=((Integer)request.getAttribute("maxpage")).intValue();
	int startpage=((Integer)request.getAttribute("startpage")).intValue();
	int endpage=((Integer)request.getAttribute("endpage")).intValue();
	String store_named = (String)request.getSession().getAttribute("store_name");
	//String store_named = (String)request.getAttribute("store_name");
	//out.println(store_named);
...
<div class="logo_183743_">
	<h1 align="center"><a align="center" href="./page/admin.ad" target="_self"><img src="./img_up/shop_pds/ehompy0118/farm/logo15196106291.jpg" alt="로고"></a></h1>
...
<div id="farmBoxLnb" class="lnb_182960_">
   ...
   <ul class="dep1">
         ...
         <a href="./page/sub1_3.jsp" target="_self">예약 관리</a>
         <div>
            <!-- 2 Depth -->
            <ul class="dep2">
                                             <li>
                  <a href="./page/sub1_3.jsp" target="_self">예약 현황</a>
               </li>
                                             <li>
                  <a href="./page/sub1_4.jsp" target="_self">1:1 문의 게시판</a>
               </li>
                           </ul>
            <!-- // 2 Depth -->
         </div>      </li>
                  <li class=" has_sub">
         <a href="./page/sub2_1.jsp" target="_self">게시판 관리</a>
         <div>
            <!-- 2 Depth -->
            <ul class="dep2">
                                             <li>
                  <a href="./page/sub2_10.jsp" target="_self">공지/뉴스 관리</a>
                                             ...
                                             <li>
                  <a href="../BoardList.bo" target="_self">자유게시판 관리</a>
               </li>                                           
                           </ul>
            <!-- // 2 Depth -->
         </div>      </li>
                  <li class=" has_sub">
         <a href="./page/sub2_6" target="_self">판매 관리</a>
         <div>
            <!-- 2 Depth -->
            <ul class="dep2">
                                             <li>
                  <a href="./page/sub2_6.jsp" target="_self">매장별 현황</a>
               </li>
                                             <li>
                  <a href="./page/sub2_7.jsp" target="_self">일자별 현황</a>
               </li>
...
<div class="img_ratio_58593_">
			<img src="./thum_img/ehompy0118/farm/reservation.jpg" alt="디너세트">
	</div>
</div></div>
	</div>
</div>
...
<div id="scbd" class="scbd co-basic">
	...
	<div id="lay_hd" class="lay_hd">
		<div class="hgroup">
			<h1><a href="./bbs_shop/list.htm?me_popup=&auto_frame=&cate_sub_idx=0&list_mode=board&board_code=g1&keyfield=&key=&page=&y=&m=">예약게시판</a></h1>
			<ul>
			<!-- 	<li>
	...
	<div id="list_board" class="list_board">
		<ul class="lst-board lst-head">
			<li>
				<div class="td col_date">예약시간</div>
				<div class="td col_date">지점명</div>
				<div class="td col_date">예약자이름</div>
				<div class="td col_date">휴대폰번호</div>
				<div class="td col_date">인원</div>
			</li>
		</ul>
		...
		int i = 0;
			...
			if(store_named.equals("전국점포")){
				for(i=0;i<ResBoardlist.size();i++){
					ReservationBean bl=(ReservationBean)ResBoardlist.get(i);
				%>
				<ul class="lst-board lst-body">
			<li class="clr">
<!-- 글번호 --> <div class="td col_date"><%=bl.getRes_date()%></div>
				<!-- <div class="td col_no2"> -->
				<!-- <div style="padding-left:0px;"> -->
				<!-- adminLogin페이지에서 로그인할때 세션에 올린 지점명을 위에서 getSession함 -->
				<div class="td col_date"><%=bl.getStore_code()%></div>
					<%-- <a href="./BoardDetailAction.bo?num=<%=bl.getStore_code()%>"></a><!--예약취소 --> --%>
				<div class="td col_date"><%=bl.getRes_name()%></div>
				<div class="td col_date"><%=bl.getRes_phone()%></div>
				<div class="td col_date"><%=bl.getRes_ppl() %></div>
				</li></ul>
			<% }
				}else{  %>
			<%for(i=0;i<ResBoardlist.size();i++){
				ReservationBean bl=(ReservationBean)ResBoardlist.get(i); %>
				<%if(bl.getStore_code().equals(store_named)){ %>
		<ul class="lst-board lst-body">
			<li class="clr">
					 <div class="td col_date"><%=bl.getRes_date()%></div>
					<!-- <div class="td col_no2"> -->
					<!-- <div style="padding-left:0px;"> -->
					<!-- adminLogin페이지에서 로그인할때 세션에 올린 지점명을 위에서 getSession함 -->
					<div class="td col_date"><%=bl.getStore_code()%></div>
			...
			<%-- <%if(i ==  ResBoardlist.size()){
				%>
				<ul class="lst-board lst-body"><li class="clr">
				<div class="td col_no_res"><%out.println("예약된 정보가 없습니다.");%></div>
			</li>
		</ul>
		<% }%> --%>
...
<%-- 		<%
		for(int i=0;i<ResBoardlist.size();i++){
			ReservationBean bl=(ReservationBean)ResBoardlist.get(i);
		if(store_name.equals(bl.getStore_code())){
		%>
		<ul class="lst-board lst-body">
			<li class="clr">
<!-- 글번호 --> <div class="td col_no2"><%=bl.getRes_date()%></div>
				<!-- <div class="td col_no2"> -->
				<!-- <div style="padding-left:0px;"> -->
				<!-- adminLogin페이지에서 로그인할때 세션에 올린 지점명을 위에서 getSession함 -->
				<div class="td col_no2"><%=bl.getStore_code()%></div>
					<a href="./BoardDetailAction.bo?num=<%=bl.getStore_code()%>"></a><!--예약취소 -->
				<div class="td col_no2"><%=bl.getRes_name()%></div>
				<div class="td col_no2"><%=bl.getRes_phone()%></div>
				<div class="td col_no2"><%=bl.getRes_ppl() %></div>
			</li>
		</ul>
		<%}else{%>
			<div class="td col_no_res"><%out.println("예약된 정보가 없습니다.");%></div>
	...
	<div class="paginate">
			<%if(nowpage<=1){ %>
			 이전&nbsp;
			<%}else{ %>	
			 <a href="./ResBoardList.yu?page=<%=nowpage-1 %>" class="dir">이전</a>&nbsp;
			<%} %>
			<%for(int a=startpage;a<=endpage;a++){
				if(a==nowpage){%>
			<strong><%=a %></strong>
				<%}else{ %>
				<a href="./ResBoardList.yu?page=<%=a %>" class="dir"><%=a %></a>&nbsp;
				<%} %>
			<%} %>
			<%if(nowpage>=maxpage){ %>
			 다음
			<%}else{ %>
			<a href="./ResBoardList.yu?page=<%=nowpage+1 %>" class="dir">다음</a>
			<%} %>			
			</div>			
										</div>	<!-- // pagenate -->
</div></div></div></div>
...
function replyArticle(idx){
						location.href="/bbs_shop/reply_form.htm?me_popup=&auto_frame=&cate_sub_idx=0&list_mode=board&board_code=g1&keyfield=&key=&page=&idx="+idx; 
}
...
function comment_go(t_iframe,idx){
	t_iframe.location.href="/bbs_shop/comment_list.htm?me_popup=&auto_frame=&cate_sub_idx=0&board_code=g1&board_idx="+idx;
}
</script>
...
function blog_file_download(board_idx,sel_no){
	...
	document.getElementById(iframe_name).src = '/bbs_shop/file_download.php?board_code=g1&board_idx='+board_idx+'&sel_no='+sel_no;
}
...
function readArticle(idx){
			...
			location.href="../bbs_shop/read.htm?me_popup=0&auto_frame=&cate_sub_idx=0&list_mode=board&board_code=g1&search_key=&key=&page=1&y=&m=&idx="+idx;
	}
function reply_readArticle(idx){
			location.href="/bbs_shop/read.htm?me_popup=0&auto_frame=&cate_sub_idx=0&list_mode=board&board_code=g1&search_key=&key=&page=1&idx="+idx;
	}
